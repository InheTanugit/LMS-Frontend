...
export default function EditCourse (props) {
    ...
    const updateCourse = async (e) => {
        ...
        let uploadImageRes
        if(image) {
            uploadImageRes = await uploadImage()
        ...
        const data = await apiconnection(`${apiEndpoints.UPDATE_COURSE_ENDPOINT}/${id}`,httpMethods.PUT,{...formData,image: image? uploadImageRes.data.data : formData.image})
  ...
  return (
    ...
    <Form>
        ...
        <Form.Group controlId="courseImage" className="mb-3">
            <Image src={image ? URL.createObjectURL(image) :formData.image} thumbnail />
            <br></br>
            <Form.Label>Upload updated image for course</Form.Label>
            <Form.Control type="file" name='courseImage' required onChange={(e) => setImage(e.target.files[0])}/>
        </Form.Group>

        <Button className='mb-3 w-100 rounded rounded-3' variant="primary" type="submit" onClick={(e)=>updateCourse(e)}>
            Update Course
        </Button>
